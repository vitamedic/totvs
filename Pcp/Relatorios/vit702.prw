...
±±³Funcao    ³ VIT318   ³ Autor ³ Alex Júnio de Miranda ³ Data ³ 18/08/08 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descricao ³ ORDEM DE PESAGEM DE MATÉRIA-PRIMA E SEPARACAO DE MATERIAL  ³±±
±±³          ³ DTecnica de Fabricação O.E                                  ±±
...
qz1->(DBSeek(xFilial("QZ1") + _cprod ))
		...
		_cquery := " SELECT"
		_cquery += " QZ1_PROD, QZ1_DOCTO, QDH_RV, QDH_CODTP, QDH_CANCEL, QDH_OBSOL"
		_cquery += " FROM "
		_cquery +=  retsqlname("QZ1")+" QZ1 "
		_cquery += " INNER JOIN QDH010 QDH ON QDH.QDH_FILIAL = QZ1_FILIAL AND QDH.QDH_DOCTO = QZ1_DOCTO"
		_cquery += " AND QDH.D_E_L_E_T_ = QZ1.D_E_L_E_T_ AND QDH.QDH_CANCEL = 'N' AND QDH.QDH_OBSOL = 'N'"
		//_cquery += " AND QDH_CODTP = '"+cTipo+"'"
		_cquery += " WHERE"
		_cquery += " QZ1.D_E_L_E_T_ <> '*'"
		_cquery += " AND QZ1_PROD  = '"+_cprod+"'"
		_cquery := changequery(_cquery)
		
		tcquery _cquery new alias "TP1"
		
		cDocC := TP1->QDH_CANCEL
		cObso := TP1->QDH_OBSOL
		
While qz1->qz1_prod == _cprod .and. lOk
	cTipo := Alltrim(Posicione("QDH",1,xfilial("QDH") + qz1->qz1_docto,"QDH_CODTP"))
	//cDocC := Alltrim(Posicione("QDH",1,xfilial("QDH") + qz1->qz1_docto,"QDH_CANCEL")) //Marcio David - 07/01/2019 Acrescentando para tratar documento cancelado
	
	If cTipo == "OE" .and. cDocC == "N" .and. cObso == "N"//Marcio David - 07/01/2019 Acrescentando .and. cDocC <> "S" para tratar documento cancelado // Guilherme Teodoro - 19/05/2016 - Melhoria para contemplar outros tipos de documentos - Projeto P.I.
		lOk := .f.
