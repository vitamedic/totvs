...
±±³Programa  ³ VIT211   ³ Autor ³ Gardenia ILany          Data ³ 27/08/04 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descricao ³ Impressao de Faturas a Pagar e Notas                       ³±±
...
user function VIT211()
nordem   :=""
tamanho  :="M"
limite   :=132
titulo   :="IMPRESSAO DE NOTAS /FATURAS A PAGAR"
cdesc1   :="Este programa ira emitir as faturas a pagar  e as notas"
cdesc2   :=""
cdesc3   :=""
cstring  :="SE2"
areturn  :={"Zebrado",1,"Administracao",2,2,1,"",1}
nomeprog :="VIT211"
wnrel    :="VIT211"+Alltrim(cusername)
alinha   :={}
nlastkey :=0
lcontinua:=.t.
...
cabec2:="FORNEC    NOME FORNECEDOR                          PRF FATURA    EMISSAO         VALOR"
...
while ! tmp1->(eof()) .and.;
	...
	while ! tmp1->(eof()) .and.;
		...
		_cquery:=" SELECT"
		_cquery+=" E2_PREFIXO PREFIXO,E2_NUM NUMERO,E2_PARCELA PARCELA,E2_TIPO TIPO,E2_VALOR VALOR"
		_cquery+=" FROM "
		_cquery+=  retsqlname("SE2")+" SE2"
		_cquery+=" WHERE"
		_cquery+="     SE2.D_E_L_E_T_<>'*'"
		_cquery+=" AND E2_FILIAL='"+_cfilse2+"'"
		_cquery+=" AND E2_FATPREF='"+tmp1->prefixo+"'"
		_cquery+=" AND E2_FATURA='"+tmp1->numero+"'"
		_cquery+=" AND E2_TIPOFAT='"+tmp1->tipo+"'"
		_cquery+=" AND E2_FORNECE='"+tmp1->fornece+"'"
		...
		tmp2->(dbgotop())
		while ! tmp2->(eof())
			_i:=ascan(_anotas,{|x| x[1]==tmp2->numero+tmp2->tipo})
			if _i==0
				sf1->(dbseek(_cfilsf1+tmp2->numero+tmp2->prefixo))
				if found()
					aadd(_anotas,{tmp2->numero,sf1->f1_valbrut})
				else	
					aadd(_anotas,{tmp2->numero,tmp2->valor})
				endif	
			endif
			tmp2->(dbskip())
		end
...
//999999-99 XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX XXX 999999999 99/99/99 9.999.999,99
	@ prow()+1,00   PSAY sa2->a2_cod+"-"+sa2->a2_loja+ " "+sa2->a2_nome
	@ prow(),51 PSAY _cprefixo
	@ prow(),55 PSAY _cnumero 
	@ prow(),65 PSAY _demissao
	@ prow(),74 PSAY _nvalor picture "@E 9,999,999.99"
...
_cquery+=" E2_FORNECE FORNECE,E2_LOJA LOJA,E2_EMISSAO EMISSAO,E2_VENCTO VENCTO,"
_cquery+=" E2_VALOR VALOR"
_cquery+=" FROM "
_cquery+=  retsqlname("SE2")+" SE2"
_cquery+=" WHERE"
_cquery+="     SE2.D_E_L_E_T_<>'*'"
_cquery+=" AND E2_FILIAL='"+_cfilse2+"'"
_cquery+=" AND E2_PREFIXO BETWEEN '"+mv_par01+"' AND '"+mv_par02+"'"
_cquery+=" AND E2_NUM BETWEEN '"+mv_par03+"' AND '"+mv_par04+"'"
_cquery+=" AND E2_EMISSAO BETWEEN '"+dtos(mv_par05)+"' AND '"+dtos(mv_par06)+"'"
_cquery+=" AND E2_FORNECE  BETWEEN '"+mv_par07+"' AND '"+mv_par09+"'"
_cquery+=" AND E2_LOJA  BETWEEN '"+mv_par08+"' AND '"+mv_par10+"'"
_cquery+=" AND E2_TIPO='FT '"
_cquery+=" ORDER BY E2_PREFIXO,E2_NUM,E2_PARCELA,E2_TIPO"
...
aadd(_agrpsx1,{cperg,"07","Do Fornecedor      ?","mv_ch7","C",06,0,0,"G",space(60),"mv_par05"       ,space(15)        ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),"SA2"})
...
aadd(_agrpsx1,{cperg,"09","Ate o Fornecedor   ?","mv_ch9","C",06,0,0,"G",space(60),"mv_par05"       ,space(15)        ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),"SA2"})
