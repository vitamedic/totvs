...
While nI <= Len(aRelat)
	...
	//Retirada tratativa pois o RECNO da SE5 a partir da versão 10 é gravado somente na posição 29 independente do título ser multinatureza
...
If Empty(NEWSE5->(E5_TIPO+E5_DOCUMEN+Iif(SE5->(FieldPos("E5_IDMOVI")) >0 , E5_IDMOVI,"")+E5_FILORIG+E5_MOEDA))
	lRet := .F.	
	//Cheque avulso gerado c/ o param mv_libcheq = N e liberado pelo Fina190"geraç de cheque"
...
aHelpPor := {"Informa se na coluna Valor Original ",;
				  ...
				  "Válido somente para o Contas a Papar."}
...
aHelpPor := {	"Define que o vencimento usado nos "	,;
				"parametros a cima será o vencimen"		,;
				...
				If (nDesc+nValor+nJurMul+nVlMovFin) > 0    
					...
					If cCarteira == "R" .and. !lManual
						...
						If !SE1->E1_TIPO $ MVRECANT+"/"+MV_CRNEG .and. Empty(SE1->E1_SALDO) .and.;
							...
							EndIf	
						ElseIf !SE1->E1_TIPO $ MVRECANT+"/"+MV_CRNEG .and. Empty(SE1->E1_SALDO) // Se não for maior seq, soma IR se na baixa 
							SA1->(DBSetOrder(1))
							If SA1->(DBSeek(xFilial("SA1")+cCliFor190) )
								lCalcIRF := SA1->A1_RECIRRF == "1" .and. SA1->A1_IRBAX == "1" // se for na baixa 
							Else
