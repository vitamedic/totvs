	          ...
	          // Posiciono na TES do cupom fiscal para pegar a natureza de operação da nf sobre cupom
						...
						/*if FindFunction( "SpedNatOper" ) .And. SFT->( MsSeek( xFilial( "SFT" ) +cChave) )
						   ...
						   Endif	
							...
							If !lNatOper
								If Empty(cNatOper)
									cNatOper := Alltrim(SF4->F4_TEXTO2)
								Else
									cNatOper += Iif(!Alltrim(SF4->F4_TEXTO2)$cNatOper,"/ " + SF4->F4_TEXTO2,"")
								Endif 
							Else	
								dbSelectArea("SX5")
								dbSetOrder(1)
...
//Customização Stephen Noel 10/08/2018 Solicitada pela contabilidade
				...
				ElseIf !lEasy .Or. (lEasy .AND. (cAliasSD1)->D1_TIPO=="D")
					...
					If MsSeek(xFilial("CD5")+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+(cAliasSD1)->D1_ITEM)
							aAdd(aDI,{;
								...
								{"S11","vTxsisc",CD5->CD5_TXSISC},;   //41          //CUSTOMIZACOO CARLA
								{"S12","vAfrm",CD5->CD5_VLAFRM},;     //42
								{"S13","vCapa",CD5->CD5_VLCAPA},;     //43
								{"S14","vTxce",CD5->CD5_VLTXCE},;     //44
								{"S15","vHono",CD5->CD5_VLHONO},;     //45
								{"S16","vArma",CD5->CD5_VLARMZ},;     //46
								{"S17","vLib",CD5->CD5_VLLIB}})	      //47
...
Endif
	...
	If nDadu > 0	
		cString += ("Valor total da Despesa Aduaneira: R$ " + ConvType(nDadu,15,2))  //CD5_DSPAD
	EndIf
		
	If nSisc > 0			
		cString += (" Valor total do Siscomex: R$ " + ConvType(nSisc,15,2))    // CD5_TXSISC
	EndIF
		
	If nSegu > 0	
		cString += (" Valor total do Seguro: R$ " + ConvType(nSegu,15,2))        //  CD5_VLSEG
	EndIf
		
	If nAfrmm > 0	
		cString += (" Valor total da AFRMM: R$ " + ConvType(nAfrmm,15,2))       // CD5_VLAFRM
	EndIf
		
	If nCapa > 0	
		cString += (" Valor total da Capatazia: R$ " + ConvType(nCapa,15,2))       //CD5_VLCAPA
	EndIf
		
	If nTxCe > 0	
		cString += (" Valor total da TX CE: R$ " + ConvType(nTxCe,15,2))              //   CD5_VLTXCE
	EndIF	
	
	If nHono > 0	
		cString += (" Valor total dos Honorários: R$ " + ConvType(nHono,15,2))      //  CD5_VLHONO
	EndIF	
	
	If nArmaz > 0	
		cString += (" Valor total da Armazenagem: R$ " + ConvType(nArmaz,15,2))  //   CD5_VLARMZ
	EndIF
	
	If nLibbl > 0	
		cString += (" Valor total da Liberação BL: R$ " + ConvType(nLibbl,15,2))      //  CD5_VLLIB
	Endif
/*If nValII > 0
