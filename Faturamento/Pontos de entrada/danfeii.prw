...
For nZ := 1 To nLenDet
	...
	// Customização para aumentar a coluna do campo descrição do produto	*****
	nMaxDes := MaxCod(NoChar(oDet[nX]:_Prod:_xProd:TEXT,lConverte), 160)
	//*************************FIM***********************************************	
	
	// Customização para apresentação das informações referentes à Lote/Fabrição/Validade/ PMC dos produtoso - Vitapan - Alex - 08/2010
    lMed:=iif(Type("oDet[nPrivate]:_Prod:_med")=="U",.f.,.t.)
	nPMC:=IIF(lMed,Val(oDet[nX]:_Prod:_med:_vPMC:TEXT),0)
	_lDI:=iif(Type("oDet[nPrivate]:_Prod:_DI")=="U",.f.,.t.)
	_nDI:=iif(_lDI,oDet[nX]:_Prod:_DI:_nDI:TEXT,"")	
	...
	AADD(aAux, AllTrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_PRCVEN")[1],TamSX3("D2_PRCVEN")[2])))) //RICARDO FIUZA'S 30/12/2014
	...
	aadd(aItens,{;
	    ...
	    SubStr(aAux[1], 1, PosQuebrVal(aAux[1])),;   //	SubStr(aAux[1], 1, 11),; 
	...
	// Customização Para colocar o Lote e data de fabricação nos itens do danfe *******************************************************************

	_cLote := AllTrim(IIF(lMed,oDet[nX]:_Prod:_med:_nLote:TEXT,""))
	
	if substr(_cLote,1,1) = "0"
    	cAux += " Lt:"+substr(_cLote,1,10)
 	else
		cAux += " Lt:"+_cLote
 	endif
    
	cAux +=	" "+AllTrim(IIF(lMed,"Fab:"+ConvDate(oDet[nX]:_Prod:_med:_dFab:TEXT,2),""))    
	if lMed .and. substr(oDet[nX]:_Prod:_med:_nLote:TEXT,1,4)=="AUTO"
		cAux +=	"             "+AllTrim(IIF(lMed,"Val:"+ConvDate(oDet[nX]:_Prod:_med:_dVal:TEXT,2),""))
		cAux +=	" "+AllTrim(IIF(lMed,"Qtd:"+AllTrim(TransForm(nQtd,TM(nQtd,TamSX3("D2_QUANT")[1],TamSX3("D2_QUANT")[2]))),""))	
	else
		cAux +=	" "+AllTrim(IIF(lMed,"Val:"+ConvDate(oDet[nX]:_Prod:_med:_dVal:TEXT,2),""))
		cAux +=	" "+AllTrim(IIF(lMed,"Qtd:"+AllTrim(TransForm(nQtd,TM(nQtd,TamSX3("D2_QUANT")[1],TamSX3("D2_QUANT")[2]))),""))
		If nPMC > 0	
		   cAux +=	" "+AllTrim(IIF(lMed,"PMC:"+AllTrim(TransForm(nPMC,TM(nPMC,TamSX3("D2_TOTAL")[1],TamSX3("D2_TOTAL")[2]))),""))
		EndIf
	endif
//************************* FIM *****************************************************************************************************************	
    lPontilhado := .F.
...
// Customização para aumentar a coluna do campo codigo do produto	*****		
		nMaxDes := MaxCod(cAux, 200)
//*******************FIM*************************************************
...
Alert("Entrei no dbseek")
_fatura:=fFatP12( SE1->E1_PREFIXO, SE1->E1_NUM, SE1->E1_PARCELA, SE1->E1_TIPO, SE1->E1_CLIENTE, SE1->E1_LOJA )
cAux:= "Duplicata(s) Aglutinada(s) na Fatura "+_fatura 
aadd(aMensagem,SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN) - 1, MAXMENLIN)))
EndIf
...
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³Quadro 1 IDENTIFICACAO DO EMITENTE CUSTOMIZADO                          ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oDanfe:Box(042,000,137,250)
oDanfe:Say(052,001, "Identificação do Emitente",oFont12N:oFont)
nLinCalc	:=	090
cStrAux		:=	AllTrim(NoChar(oEmitente:_xNome:Text,lConverte))
nForTo		:=	Len(cStrAux)/50
nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
For nX := 1 To nForTo
	oDanfe:Say(nLinCalc,001,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*50)+1),50), oFont12N:oFont )
	nLinCalc+=7
Next nX

If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
	cStrAux		:=	AllTrim(NoChar(oEmitente:_EnderEmit:_xLgr:Text,lConverte))
	If right(AllTrim(NoChar(oEmitente:_EnderEmit:_xLgr:Text,lConverte)),4)==", SN"
		cStrAux		+=	", "+AllTrim(NoChar(oEmitente:_EnderEmit:_xCpl:TEXT,lConverte))
	Else                
		cStrAux		+=	", "+AllTrim(oEmitente:_EnderEmit:_Nro:Text)+", "+AllTrim(NoChar(oEmitente:_EnderEmit:_xCpl:TEXT,lConverte))
	Endif
Else
	If right(AllTrim(NoChar(oEmitente:_EnderEmit:_xLgr:Text,lConverte)),4)==", SN"
		cStrAux		:=	AllTrim(NoChar(oEmitente:_EnderEmit:_xLgr:Text,lConverte))
...
nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
For nX := 1 To nForTo
	oDanfe:Say(nLinCalc,001,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*45)+1),45),oFont09:oFont)
	nLinCalc+=7
Next nX
	...
	nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
	For nX := 1 To nForTo
		oDanfe:Say(nLinCalc,001,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*32)+1),32),oFont09:oFont)
		nLinCalc+=7
	Next nX
	oDanfe:Say(nLinCalc,001, oEmitente:_EnderEmit:_xMun:Text+"/"+oEmitente:_EnderEmit:_UF:Text,oFont09:oFont)
	nLinCalc+=7
	oDanfe:Say(nLinCalc,001, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont09:oFont)
...
If lMv_Logod
   ...
   //	If !Empty(cUnitGrp)
	 //	cDescLogo	:= cGrpCompany + cCodEmpGrp + cUnitGrp + cFilGrp  Ricardo Fiuza's 17/03/2015
	//Else
	...
	EndIf
EndIf 
//****Customizado para acertar o logo ************************************************ Ricardo Fiuza's 17/03/2015
If nfolha==1
	If lMv_Logod
		oDanfe:SayBitmap(052,001,cLogoD,072,027)
	Else
		oDanfe:SayBitmap(052,001,cLogo,072,027)
	EndIF
Endif

// Trexo retirado era oque veio no padrão ********************************************
/*If nfolha==1
	If lMv_Logod
...
oDanfe:Box(442,000,678,603)
nAuxH := 0
oDanfe:Box(442, nAuxH, 678, nAuxH + 30)
oDanfe:Say(450, nAuxH + 2, "CODIGO",oFont08N:oFont)
nAuxH += aTamCol[1]
oDanfe:Say(450, nAuxH -18, "DESCRIÇÃO DO PROD./SERV.", oFont08N:oFont) 
nAuxH += aTamCol[2]
oDanfe:Box(442, nAuxH - 80, 678, nAuxH + aTamCol[3] - 78)
oDanfe:Say(450, nAuxH - 73, "NCM/SH", oFont08N:oFont)
nAuxH += aTamCol[3] 
oDanfe:Box(442, nAuxH - 60, 678, nAuxH + aTamCol[4] - 55)
oDanfe:Say(450, nAuxH - 76, "CST", oFont08N:oFont)
nAuxH += aTamCol[4]
oDanfe:Say(450, nAuxH - 73, "CFOP", oFont08N:oFont)
nAuxH += aTamCol[5]
oDanfe:Say(450, nAuxH - 74, "UN", oFont08N:oFont)
nAuxH += aTamCol[6]
oDanfe:Box(442, nAuxH - 75, 678, nAuxH + aTamCol[7] - 55)
oDanfe:Say(450, nAuxH - 60, "QUANT.", oFont08N:oFont)
nAuxH += aTamCol[7]
oDanfe:Say(450, nAuxH  - 43, "V.UNITARIO", oFont08N:oFont)
nAuxH += aTamCol[8]
oDanfe:Box(442, nAuxH - 51, 678, nAuxH + aTamCol[9] - 45)
oDanfe:Say(450, nAuxH - 40, "V.TOTAL", oFont08N:oFont)
nAuxH += aTamCol[9]
oDanfe:Say(450, nAuxH - 35, "BC.ICMS", oFont08N:oFont)
nAuxH += aTamCol[10]
oDanfe:Box(442, nAuxH - 33, 678, nAuxH + aTamCol[11] - 25)
oDanfe:Say(450, nAuxH - 25, "V.ICMS", oFont08N:oFont)
nAuxH += aTamCol[11]
oDanfe:Box(442, nAuxH - 25, 678, nAuxH + aTamCol[12] - 5)
oDanfe:Say(450, nAuxH - 15, "V.IPI", oFont08N:oFont)
nAuxH += aTamCol[12]
oDanfe:Box(442, nAuxH - 5, 678, nAuxH + aTamCol[13] - 7)
oDanfe:Say(450, nAuxH - 3, "A.ICMS", oFont08N:oFont)
nAuxH += aTamCol[13]
oDanfe:Box(442, nAuxH - 7, 678, nAuxH + aTamCol[14])
oDanfe:Say(450, nAuxH, "A.IPI", oFont08N:oFont)
...
For nY := 1 To nLenItens
	...
	If lflag
		If nL > nMaxItemP2
			...
			oDanfe:Say(010+nLinhavers,001, "Identificação do emitente",oFont12N:oFont)
		...
		oDanfe:Say(nLinCalc,001, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont09:oFont)
			...
			*/
			...
			If lMv_Logod
				oDanfe:SayBitmap(010+nLinhavers,001,cLogoD,072,027)
			Else
				oDanfe:SayBitmap(010+nLinhavers,001,cLogo,072,027)
			EndIf
	...
	If	nL > Iif( (nfolha-1)%2==0 .And. MV_PAR05==1,MAXITEMP3,nMaxItemP2)
		...
		oDanfe:Say(010,001, "Identificação do emitente",oFont12N:oFont)
		/*
		nLinCalc	:=	023
		cStrAux		:=	AllTrim(NoChar(oEmitente:_xNome:Text,lConverte))
		nForTo		:=	Len(cStrAux)/25
		...
		*/
		...
		If lMv_Logod
			oDanfe:SayBitmap(010,001,cLogoD,072,027)
		Else
			oDanfe:SayBitmap(010,001,cLogo,072,027)
		EndIf
 ...
 */        
 If aAux[1][1][nY] == "-"
	...
	Else
		oDanfe:Say(nLinha, nAuxH + 2, aAux[1][1][nY], oFont08:oFont )
		nAuxH += aTamCol[1]
		oDanfe:Say(nLinha, nAuxH -16, substr(aAux[1][2][nY],1,40), oFont08:oFont) // DESCRICAO DO PRODUTO        //ricardo 08042016
		//oDanfe:Say(nLinha, nAuxH + 2, aAux[1][2][nY], oFont08:oFont) // DESCRICAO DO PRODUTO ricardo fiuza 30/12/2014
		nAuxH += aTamCol[2]
		oDanfe:Say(nLinha, nAuxH - 78, aAux[1][3][nY], oFont08:oFont) // NCM 
		//oDanfe:Say(nLinha, nAuxH + 2, aAux[1][3][nY], oFont08:oFont) // NCM
		nAuxH += aTamCol[3]
		oDanfe:Say(nLinha, nAuxH - 76, aAux[1][4][nY], oFont08:oFont) // CST
//		oDanfe:Say(nLinha, nAuxH + 2, aAux[1][4][nY], oFont08:oFont) // CST
		nAuxH += aTamCol[4]
		oDanfe:Say(nLinha, nAuxH - 73, aAux[1][5][nY], oFont08:oFont) // CFOP
		//oDanfe:Say(nLinha, nAuxH + 2, aAux[1][5][nY], oFont08:oFont) // CFOP
		nAuxH += aTamCol[5]
		oDanfe:Say(nLinha, nAuxH - 75, aAux[1][6][nY], oFont08:oFont) // UN
//		oDanfe:Say(nLinha, nAuxH + 2, aAux[1][6][nY], oFont08:oFont) // UN
		nAuxH += aTamCol[6]
		// Workaround para falha no FWMSPrinter:GetTextWidth()
		...
		EndIf
		oDanfe:Say(nLinha, nAuxH2 - 52, aAux[1][7][nY], oFont08:oFont) // QUANT
	//	oDanfe:Say(nLinha, nAuxH2 + 2, aAux[1][7][nY], oFont08:oFont) // QUANT
		...
		EndIf
	   	oDanfe:Say(nLinha, nAuxH2 - 47,aAux[1][8][nY], oFont08:oFont) // V UNITARIO
		...
		EndIf
		oDanfe:Say(nLinha, nAuxH2 - 43, aAux[1][9][nY], oFont08:oFont) // V. TOTAL 
	   //oDanfe:Say(nLinha, nAuxH2 + 2, aAux[1][9][nY], oFont08:oFont) // V. TOTAL
		...
		EndIf
		oDanfe:Say(nLinha, nAuxH2 - 32, aAux[1][10][nY], oFont08:oFont) // BC. ICMS
		...
		EndIf
		oDanfe:Say(nLinha, nAuxH2 - 23, aAux[1][11][nY], oFont08:oFont) // V. ICMS
		...
		EndIf
		oDanfe:Say(nLinha, nAuxH2 - 5, aAux[1][12][nY], oFont08:oFont) // V.IPI
		nAuxH += aTamCol[12]
		nAuxH2 := nAuxH + ((aTamCol[13] - 2) - RetTamTex(aAux[1][13][nY], oFont08:oFont, oDanfe)) + 2
		oDanfe:Say(nLinha, nAuxH2 - 5, aAux[1][13][nY], oFont08:oFont) // A.ICMS
		nAuxH += aTamCol[13]
		nAuxH2 := nAuxH + ((aTamCol[14] - 2) - RetTamTex(aAux[1][14][nY], oFont08:oFont, oDanfe)) + 2
		oDanfe:Say(nLinha, nAuxH2 + 4, aAux[1][14][nY], oFont08:oFont) // A.IPI
	EndIf
...
//================================================================================
static function fFatP12( cPrefFAT, cNumFAT, cParcFAT, cTipFAT, cCliFAT, cLojFAT )
//===============================================================================

local aArea		:= getArea()
local aAreaSE1	:= SE1->( getArea() ) 
local aAreaFO0	:= FO0->( getArea() )
local aAreaFO2	:= FO2->( getArea() )
local cRet 		:= ""
local cQuery 	:= ""
...
// query SE5
cQuery := " SELECT DISTINCT(E5_DOCUMEN) NUMLIQ FROM " + retSqlName("SE5") + " SE5 	"
cQuery += " WHERE SE5.D_E_L_E_T_ = ' ' 						"
cQuery += " AND SE5.E5_FILIAL 	= '" + xFilial("SE5") + "'	"
cQuery += " AND SE5.E5_PREFIXO 	= '" + cPrefFAT + "'		"
cQuery += " AND SE5.E5_NUMERO	= '" + cNumFAT	+ "'		"
cQuery += " AND SE5.E5_PARCELA	= '" + cParcFAT	+ "'		"
cQuery += " AND SE5.E5_TIPO		= '" + cTipFAT	+ "' 		"
cQuery += " AND SE5.E5_CLIFOR	= '" + cCliFAT	+ "'		"
cQuery += " AND SE5.E5_LOJA		= '" + cLojFAT	+ "'		"
cQuery += " AND SE5.E5_MOTBX	= 'LIQ'						"
cQuery += " AND SE5.E5_ORIGEM   = 'FINA460'					"
...
if TRBSE5->( !eof() )
	...
	// query FO0
	cQuery := ""
	cQuery := " SELECT DISTINCT(FO0_PROCES) PROCES FROM " + retSqlName("FO0") + " FO0 	"
	cQuery += " WHERE FO0.D_E_L_E_T_ = ' '   											"
	cQuery += " AND FO0.FO0_FILIAL  = '" + xFilial("FO0") + "'							"
	cQuery += " AND FO0.FO0_NUMLIQ 	= '" + TRBSE5->NUMLIQ + "'                  		"
	cQuery += " AND FO0.FO0_CLIENT	= '" + cCliFAT 	+ "'								"
	cQuery += " AND FO0.FO0_LOJA	= '" + cLojFAT	+ "'								"
	cQuery += " AND FO0.FO0_ORIGEM	= 'FINA460'											"
	
	tcQuery cQuery new alias "TRBFO0"
	...
	if TRBFO0->( !eof() )
		...
		// query FO2
		cQuery := ""
		cQuery := " SELECT FO2_PREFIX, FO2_NUM FROM " + retSqlName("FO2") + " FO2 		"
		cQuery += " WHERE FO2.D_E_L_E_T_ = ' '                                          "
		cQuery += " AND FO2.FO2_FILIAL 	= '" + xFilial("FO2") + "'						"
		cQuery += " AND FO2.FO2_PROCES	= '" + TRBFO0->PROCES + "'						"
		cQuery += " GROUP BY FO2_PREFIX, FO2_NUM 										"
		
		tcQuery cQuery new alias "TRBFO2"
		
		if TRBFO2->( !eof() )
			cRet := TRBFO2->FO2_PREFIX + " - " + TRBFO2->FO2_NUM // retorno o prefixo e numero
		endIf
