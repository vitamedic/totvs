...
±±³Programa  ³ VIT171 ³ Autor ³ Gardenia Ilany        ³ Data ³ 20/11/03   ³±±
...
while ! sa3->(eof())
	...
	while ! tmp1->(eof()) 
		...
		if !_lpassou  
			_carq:=_vend+"."+substr(tmp1->pocket,3,8)+"."+strzero(val(tmp1->num),7,0)+".RET"
...
//		endif	
		...
		_lpassou:=.f. 
		if !_lpassou  
			_carq:=tmp2->vend1+"."+substr(_pocket,3,8)+"."+strzero(val(tmp2->doc),7,0)+".ITN"
			_nhandle:=fcreate(_carq,0)
			frename(_carq,"\pocket\retornos\"+_carq)
			if _nhandle<0
				msginfo("Erro na criacao do arquivo "+_carq)
		...
		while ! tmp2->(eof())  .and. _nota== tmp2->doc
			...
			fwrite(_nhandle,strzero(round(tmp2->quant*100,0),9)) 
...
tcsetfield("TMP2","QUANT"  ,"N",15,3)
tcsetfield("TMP2","PRCVEN"  ,"N",17,6)
return
...
aadd(_agrpsx1,{_cperg,"07","Da data nota       ?","mv_ch7","D",08,0,0,"G",space(60),"mv_par07"       ,space(15)        ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),"   "})
aadd(_agrpsx1,{_cperg,"08","Ate a data nota    ?","mv_ch8","D",08,0,0,"G",space(60),"mv_par08"       ,space(15)        ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),"   "})
