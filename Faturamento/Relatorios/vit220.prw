...
±±³Programa  ³ VIT220   ³ Autor ³ Aline Borges Pereira  ³ Data ³ 23/12/04 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descricao ³ Produtos por Cliente e Nota                                ³±±
...
user function vit220()
nordem  :=""
tamanho :="P"
limite  :=80
titulo  :="CLIENTES E PRODUTOS "
cdesc1  :="Este programa ira emitir a relacao de Clientes e produtos"
cdesc2  :=""
cdesc3  :=""
cstring :="SD2"
areturn :={"Zebrado",1,"Administracao",1,2,1,"",1}
nomeprog:="VIT220"
wnrel   :="VIT220"+Alltrim(cusername)
alinha  :={}
aordem  :={}
nlastkey:=0
...
cabec2:="Codigo Descricao            Quantidade  Lote  Validade   Doc.  Emissao"
// nome cliente:
// produto descricao  lote validade nota emissao
...
while ! tmp1->(eof()) .and.;
	...
	if _ccli <> tmp1->cliente             
   	_ccli:=tmp1->cliente
		sa1->(dbseek(_cfilsa1+_ccli))
      _ntipo:=tmp1->tipo
		@ prow()+2,000 PSAY "CLIENTE: "+_ccli+" - "+sa1->a1_nome
   endif  
...
//   	_impzero()
   ...
   sb8->(dbseek(_cfilsb8+tmp1->produto+tmp1->lote))
	@ prow()+1,000   PSAY left(tmp1->produto,6)
	@ prow(),007   PSAY substr(tmp1->descricao,1,20)
	@ prow(),029   PSAY tmp1->quant picture "@E 999,999,999"	
	@ prow(),041   PSAY tmp1->lote picture "@E 999999"
	@ prow(),048   PSAY sb8->b8_dtvalid
	@ prow(),059   PSAY tmp1->documento
	@ prow(),066   PSAY tmp1->emissao	
...
while ! sd2010->(eof()) .and.;
	...
	if sd2010->d2_cod>=mv_par07 .and.;
		sd2010->d2_cod<=mv_par08
		sa1010->(dbseek(_cfilsa1+sd2010->d2_cliente+sd2010->d2_loja))
...
//			_cvend:=sf2010->(fieldget(fieldpos("F2_VEND"+alltrim(str(mv_par09,1)))))
			...
			if sd2010->d2_cliente>=mv_par03 .and.;
				sd2010->d2_cliente<=mv_par04 .and.;
				sd2010->d2_loja>=mv_par05 .and.;
				sd2010->d2_loja<=mv_par06								
				if tmp1->(dbseek(sd2010->d2_cliente+sd2010->d2_loja+sd2010->d2_cod))
						tmp1->quant+=sd2010->d2_quant
				else                             
					tmp1->(dbappend())           
					tmp1->cliente :=sd2010->d2_cliente      
					tmp1->loja     :=sd2010->d2_loja      
					tmp1->descr    :=sa1->a1_nome					
					tmp1->descricao:=sb1->b1_desc
					tmp1->tipo     :=sb1->b1_apreven
					tmp1->produto  :=sd2010->d2_cod
					tmp1->quant    :=sd2010->d2_quant
					tmp1->lote     :=sd2010->d2_lotectl									
					tmp1->documento:=sd2010->d2_doc						
...
/*	if sd2->d2_cod>= mv_par07 .and.;
		sd2->d2_cod<=mv_par08*/
	if substr(sd2->d2_cod,1,6)= "000710" .OR.;
		substr(sd2->d2_cod,1,6)= "000131"
...
//			_cvend:=sf2->(fieldget(fieldpos("F2_VEND"+alltrim(str(mv_par09,1)))))
			...
			if sd2->d2_cliente>=mv_par03 .and.;
				sd2->d2_cliente<=mv_par04 .and.;
				sd2->d2_loja>=mv_par05 .and.;
				sd2->d2_loja<=mv_par06				
				sa1->(dbseek(_cfilsa1+sd2->d2_cliente+sd2->d2_loja))
				if tmp1->(dbseek(sd2->d2_cliente+sd2->d2_loja+sd2->d2_cod))
					...
					tmp1->quant    :=sd2->d2_quant
					tmp1->lote     :=sd2->d2_lotectl
					tmp1->emissao  :=sd2->d2_emissao					
					tmp1->documento:=sd2->d2_doc										
				endif
			endif
		endif
	sd2->(dbskip())
...
aadd(_agrpsx1,{cperg,"09","Salta pagina       ?","mv_ch9","N",01,0,0,"C",space(60),"mv_par09"       ,"Sim"            ,space(30),space(15),"Nao"            ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),"   "})
aadd(_agrpsx1,{cperg,"10","Grade              ?","mv_chA","C",01,0,0,"G",space(60),"mv_par10"       ,"1"              ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),"   "})
