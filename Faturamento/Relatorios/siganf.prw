...
User Function SIGANF()        // incluido pelo assistente de conversao do AP5 IDE em 19/11/99
...
SetPrvt("XV_ICMS_SOL,NCONT,NCOL,NTAMOBS,NAJUSTE,BB,_NCASAS,xTPCLIEN,xTPEMP,xTIPO,xTPMAT")
...
If mv_par04 == 2
	...
	While !eof() .and. SF2->F2_DOC    <= mv_par02 .and. lContinua
		...
		while !eof() .and.  SD2->D2_DOC==  xNUM_NF .and. SD2->D2_SERIE==xSERIE
			...
			//Acumula total do itens sem o desconto para impressão de mensagem em NF Saída do MT
			_totreduzmt+= sd2->d2_quant*SD2->D2_PRUNIT
			
			_ncasas:=0
			_mlic := .f.
			xTPMAT := if((sb1->b1_tipo="PA"),.t.,.f.)
			if sc5->c5_desc3>0
				...
				// CALCULO PRECO MAX.CONS
				_mzfm:= .f.
				_fatorpos:=0.7234
				if sf2->f2_est = "RJ"
					_fatorneg:=0.7523
				elseif sf2->f2_est = "SP#RS#MG#SC#PR"
					_fatorneg:=0.7519
				else
					_fatorneg:=0.7515
				endif
				if sc5->c5_licitac <> "S"
					if sb1->b1_categ=="N  " .and. da0->da0_status<>"Z"
						_maxcons:=sd2->d2_prunit/_fatorneg
					else
						_maxcons:=sd2->d2_prunit/_fatorpos
						if da0->da0_status="Z" .and. sb1->b1_categ=="N  "
							_mzfm:= .t.
						endif
					endif
				else
					_mlic := .t.
					if sb1->b1_categ=="N  " .and. da0->da0_status<>"Z"
						_maxcons:=sd2->d2_prcven/_fatorneg
					else
						_maxcons:=sd2->d2_prcven/_fatorpos
						if da0->da0_status="Z" .and. sb1->b1_categ=="N  "
							_mzfm:= .t.
						endif
					endif
				...
				if sd2->d2_desczfr>0
					...
					_npreco:=round(_ntotal/_nquant,6)
			...
			elseif sc5->c5_cliente == "394700" // sc5->c5_desc3>0
				...
				if sc5->c5_licitac <> "S"
					if (sb1->b1_categ=="N  ") .and. (da0->da0_status<>"Z")
						if sb1->b1_tipo == "PA"   // coloquei para pegar apenas produtos do tipo PA - Gardênia 03/03/05
							_maxcons:=sd2->d2_prunit/_fatorneg
						endif
					else
						if !empty(sb1->b1_categ)
							_maxcons:=sd2->d2_prunit/_fatorpos
						endif
						if (da0->da0_status="Z") .and. (sb1->b1_categ=="N  ")
							_mzfm:= .t.
					...
					if sb1->b1_categ=="N  " .and. da0->da0_status<>"Z"  
						...
						if (sa1->a1_suframa<>' ') // Incluído para alterar notas de Licitação para Clientes ZFM utilizando-se tabela de Licitações (DA0_STATUS='L')
							_mzfm:= .t.
							_maxcons:=sd2->d2_prcven/_fatorpos												
						endif
		...
		For I:=1 to Len(xCOD_PRO)
			...
			If Empty(mv_par07)
				AADD(xDESCRICAO ,{SB1->B1_DESC,I}) // NOTA DE SAIDA
			else
				dbSelectArea("SC6")
				dbSetOrder(1)
				dbseek(xfilial("SC6")+xPED_VEND[i]+xITEM_PED[i]+xCOD_PRO[I])
				AADD(xDESCRICAO,{SC6->C6_DESCRI,I}) // NOTA DE SAIDA
			endif
		...
		If xTIPO=='N' .OR. xTIPO=='C' .OR. xTIPO=='P' .OR. xTIPO=='I' .OR. xTIPO=='S' .OR. xTIPO=='T' .OR. xTIPO=='O'
			...
			xTPEMP   := SA1->A1_TPEMP // TIPO EMPRESA
			xTIPO    := SA1->A1_TIPO // TIPO CONSUMIDOR
		...
		xNATUREZA:=SF4->F4_TEXTO2             // Natureza da Operacao - Substitui linha abaixo, buscando dados do campo criado
...
Else  // nota de entrada
	...
	While !eof() .and. SF1->F1_DOC <= mv_par02 .and. SF1->F1_SERIE == mv_par03 .and. lContinua
		...
		For I:=1 to Len(xCOD_PRO)
			...
			if empty(mv_par07)
				AADD(xDESC_PRO ,SB1->B1_DESC)     // DESCRIÇÃO CADASTRO PRODUTO
				...
				AADD(xDESC_PRO ,SD1->D1_DESCPRO)  // DESCRIÇÃO LANÇADA NA NF
				AADD(xDESCRICAO ,{SD1->D1_DESCPRO,I})
			end
...
For I:=1 to len(xdescsort)
	_npos:=xdescsort[i,2]
	@ nLin, 024  PSAY SUBSTR(xCOD_PRO[_npos],1,6)
	@ nLin, 033  PSAY xDESCSORT[I,1]
	@ nLin, 084  PSAY left(xLOTE[_npos],10)
	@ nLin, 096  PSAY xCOD_FIS[_npos]
	
	@ nLin, 106  PSAY xCOD_TRIB[_npos]
	@ nLin, 111  PSAY xUNID_PRO[_npos]
	if xTIPO_PRO[_npos] ==	"MP"
		@ nLin, 115  PSAY xQTD_PRO[_npos]               Picture "@E 9999999.999"
	else
		if xtipoped <> "B" .and. xtipoped <> "D"
			@ nLin, 115  PSAY xQTD_PRO[_npos]               Picture "@E 999999999"
		else
			@ nLin, 115  PSAY xQTD_PRO[_npos]               Picture "@E 999999999"
		endif
	endif
	...
	//		@ nLin, 125  PSAY xPRE_TAB[_npos]               Picture"@E 999,999.999999"
	//		@ nLin, 161  PSAY _NTOT                 Picture"@E 999,999.999999"
	//   else
	@ nLin, 125  PSAY xPRE_UNI[_npos]               Picture "@E 999,999.999999"
	if mv_par04 == 2
		if !_mlic  .and. xTIPO_PRO[_npos] ==	"PA" .or. !empty(xpre_tab[_npos])
			@ nLin, 141  PSAY xPRE_TAB[_npos]               Picture"@E 999,999.99" // PRECO MAX.CONS
		elseif !empty(xpre_tab[_npos])
			if szl->zl_umprop =="2"
				@ nLin, 141  PSAY xPRE_TAB[_npos]               Picture"@E 999,999.999999" // PRECO MAX.CONS
			else
				@ nLin, 141  PSAY xPRE_TAB[_npos]               Picture"@E 999,999.99" // PRECO MAX.CONS
			endif
		endif
	endif
	@ nLin, 161  PSAY xVAL_MERC[_npos]              Picture"@E 99,999,999.99"
	//	endif
	@ nLin, 176  PSAY xICM_PROD[_npos]              Picture"99"
	...
	if j==28
		...
		@ 68, 155 PSAY chr(14)+xNUM_NF+chr(20)                   // Numero da Nota Fiscal
		
		@ 069, 000 PSAY chr(18)                   // Descompressao de Impressao
		@ 072,000  PSAY " "
...
if _ndesczfr>0
	nlin++
	@ nlin,131 PSAY "Desc. ref. ICMS/SUFRAMA 12%: "+transform(_ndesczfr,"@E 99,999,999.99")
	nlin++
	@ nlin,25  PSAY "DESC.ICMS/SUFRAMA ART.VI INC.XVII ANEXO IX DO RCTE DEC.4852/97 - COD.AGENFA ORIGEM 91200121."
	if _mzfm
		nlin++
		@ nlin,25  PSAY "FATURADO CONFORME ART.2o DA LEI 10.996, DE 15 DE DEZEMBRO DE 2004(COMUNICADO DA ANVISA No.12,"
		nlin++
		@ nlin,25  PSAY " DE 02 DE JUNHO DE 2005.)"
	endif
endif

// As linhas de código abaixo foi inativada conforme Comunicação Interna 11/2008, de 12/06/2008 enviada pela Contabilidade
/*
if (mv_par04==2) .and.;		// NF Saída
(xEST_CLI=="MT") //.and.;	// Descrição válida para estado do MT

// A linha de código abaixo foi inativada conforme chamado Ocomon nº 2200/07, de 06/12/07
//	(xEMISSAO <= ctod("31/12/06"))		// Data de término da vigência do ofício, conforme CI nº128/06, de 18/10/06

nlin+=2
@ nlin,25  PSAY "PARA EFEITO DE RECOLHIMENTO DO ICMS GARANTIDO INTEGRAL, NA ENTRADA DE MEDICAMENTOS NO ESTADO DO"
nlin++
@ nlin,25  PSAY "MATO GROSSO, CONFORME LEGISLACAO LOCAL E OF. CIRCULAR Nº 027/06/GAB/CGFIS DE 13/09/06, A BASE"
nlin++
@ nlin,25  PSAY "DE CALCULO DESTA NF E DE R$ "+transform((_totreduzmt*0.40),"@E 99,999,999.99")
nlin++
endif
*/

// As linhas de código abaixo foram criadas conforme e-mail recebido em 18/07/2008, enviado pela Contabilidade

if (mv_par04==2) .and.;		// NF Saída
	(xEST_CLI=="MT") //.and.;	// Descrição válida para estado do MT

	nlin+=2
	@ nlin,25  PSAY "REGIME DE SUBSTITUICAO TRIBUTARIA, COM RESPONSABILIDADE DE RECOLHIMENTO DO ICMS PELO DESTINATARIO,"
	nlin++
	@ nlin,25  PSAY "  CONFORME ANEXO XIV, ART.3o., DO DECRETO 1944/89"
	nlin+=2
	@ nlin,25  PSAY "Medicamentos GENERICOS E/OU SIMILARES, conforme artigo 37 do ANEXO"
	nlin++
	@ nlin,25  PSAY "  VIII do RICMS, paragrafos 2o. e 3o., introduzido pelo Decreto 1388/2008"
	nlin++                           	
endif


if (mv_par04==2) .and.;					// NF Saída
	xTPMAT .and.;						// TIPO MATERIAL IGUAL A PA
	((!empty(xTPEMP) .and. xTIPO ="F")) // tipo igual a consumidor final e tipo cliente igual a M(municipal),E(estadual),F(federal)
	// A linha de código abaixo foi inativada conforme chamado Ocomon nº 2210/07, de 10/12/07
	//	((xEST_CLI=="GO") .or.;			// Descrição válida para estado de GO
	if xDUPLICATAS==.T. 				// Mensagem aplicada somente para Operações de Vendas, excluindo-se bonificações/brindes/Doações (sem financeiro)
		nlin+=2
		// A linha de código abaixo foi alterada conforme e-mail recebido do fiscal em 11/08/2008.
		//@ nlin,25  PSAY "BASE DE CALCULO REDUZIDA CONF.ART.8o INC.VIII, ANEXO IX RCTE-GO E TARE 347/06-GSF"
		@ nlin,25  PSAY "BASE DE CALCULO REDUZIDA CONF.ART.8o INC.VIII, ANEXO IX RCTE-GO"
		nlin++
	endif
endif
nlin++
...
If !Empty(MV_PAR06)
	...
	nlin++
Endif
if (sc5->c5_licitac =="S") .AND. (mv_par04 == 2) .AND.;
	(sc5->c5_tipo<>"D")
	...
	nlin++
	nlin++
	@ nlin,025 PSAY "VENDA PROIBIDA AO COMERCIO VAREJISTA"
endif
...
//INCLUÍDO CONFORME SOLICITAÇÃO PARA UTILIZAÇÃO DE FORMULÁRIOS ANTIGOS ATÉ 18/05/08 OU FIM DE ESTOQUE
//17/05/2006 - CHAMADO OCOMON Nº 1357.
//  nlin++
//  nlin++
//  @ nlin,25  PSAY "DATA LIMITE PARA UTILIZACAO, PRORROGADA ATE 18/05/2008, CONFORME DESPACHO NR. 61692-3 SEFAZ."
// LINHAS ANTERIORES COLOCADAS COMO COMENTÁRIO APÓS TÉRMINO DE UTILIZAÇÃO DE FORMULÁRIOS ANTIGOS. ALEX - 31/05/2006.


//INCLUÍDO CONFORME SOLICITAÇÃO PARA UTILIZAÇÃO DE FORMULÁRIOS ANTIGOS ATÉ 10/03/09 OU FIM DE ESTOQUE
//19/03/2008 - CHAMADO OCOMON Nº 2443.
//nlin++
//nlin++
//@ nlin,025 PSAY "DATA LIMITE PARA UTILIZACAO, PRORROGADA ATE 10/03/2009, CONFORME DESPACHO NR. 81384-2 SEFAZ."
// LINHAS ANTERIORES COLOCADAS COMO COMENTÁRIO APÓS TÉRMINO DE UTILIZAÇÃO DE FORMULÁRIOS ANTIGOS. ALEX - 01/04/2008.
...
@ 49, 061  PSAY xSEGURO     Picture "@E@Z 999,999,999.99"  // Valor Seguro
@ 49, 085  PSAY xDESPESA    Picture "@E@Z 999,999,999.99"  // Valor Despesa
...
If mv_par04 == 2
	@ 60,025 PSAY sa1->a1_cod+"/"+sa1->a1_loja
	@ 60,053 PSAY xped_vend[1]
	@ 60,090 PSAY xcod_vend[1]
	If _ndesczfr>0
		@ 62,25 PSAY _ndesczfr picture "@E 999,999,999.99"
		@ 62,40 PSAY "Codigo SUFRAMA: "+xsuframa
	EndIf
