...
±±³Funcao    ³ VIT088   ³ Autor ³ Gardenia Ilany        ³ Data ³ 11/07/02 ³±±
...
Tamanho:= "G"
limite := 200 
titulo := "IMPRESSAO DE ETIQUETAS DE VENDEDORES"
cDesc1 := oemtoansi("Este programa ira emitir as etiquetas de vendedores                   ")
cDesc2 := ""
cDesc3 := ""
...
wnrel:="VIT088"+Alltrim(cusername)
...
sa3->(dbseek(cfilsa3+mv_par01,.t.))
aetiq1:={"","","","",""}
aetiq2:={"","","","",""}
aetiq3:={"","","","",""}
aetiq4:={"","","","",""}
i:=1
while ! sa3->(eof()) .and. sa3->a3_cod<=mv_par02
	if ((mv_par03<>2) .and. (sa3->a3_ativo=="S")) .or.;
		((mv_par03<>1) .and. (sa3->a3_ativo=="N"))
		
		_nome:=sa3->a3_nome
		_end:=sa3->a3_end
		_bairro:=sa3->a3_bairro
		_est:=sa3->a3_est
		_mun:=sa3->a3_mun
		_cep:=sa3->a3_cep
		if i==1
...
aadd(_agrpsx1,{cperg,"03","Imprime Vendedor   ?","mv_ch3","N",01,0,0,"C",space(60),"mv_par03"       ,"Ativo"          ,space(30),space(15),"Inativo"        ,space(30),space(15),"Ambos"          ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),"   "})
