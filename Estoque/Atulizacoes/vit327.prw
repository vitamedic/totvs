...
±±³Programa  ³ VIT327   ³ Autor ³ Reuber A. Moura Junior³ Data ³ 22/08/08 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descricao ³ Gera Tabela de Saldo Retroativo na Data Informada          ³±±
±±³          ³ Tabela SZR                                                 ³±±
...
user function vit327()
cperg:="PERGVIT327"
_pergsx1()  
            
if pergunte(cperg,.t.) .and.;
	msgyesno("Confirma a Geração do Saldo em "+mv_par01+"/"+mv_par02+"?")

	cDataref:=lastday(ctod("01/"+strzero(val(mv_par01),2)+"/"+strzero(val(mv_par02),4)))
	cData:=strzero(year(cDataref),4)+strzero(month(cDataref),2)+strzero(day(cDataref),2)
	alert(cData)
...
#IFDEF TOP
	...
	If ExistProc( cProcedure )
		TCSPEXEC( xProcedures(cProcedure),cData)
	endif
#ENDIF
