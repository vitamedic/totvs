...
±±³Programa  ³ VIT427    ³ Autor ³ Guilherme Teodoro³     Data ³ 16/09/15 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descricao ³ Impressao de etiquetas com codigo de barras - Recebimento  ³±±
...
#IFDEF TOP
	...
	BeginSql Alias tmp1
		...
		SD1.D1_FABRIC FABRIC,      //Guilherme 16/09/2015
		SD1.D1_LOTFABR LOTFABR     //Guilherme 16/09/2015
		FROM
		%Table:SD1% SD1
...
while ! (tmp1)->(eof()) 
	...
	if (tmp1)->tipo=="MP"
		...
		for _i:=1 to _volume
			...
			mscbbegin(1,6)
			mscbsay(010,003,"VITAMEDIC IND. FARMACEUTICA LTDA","N","0","050,028") // TÍTULO
//			mscbsay(085,003,alltrim(strzero(_i,3))+"/"+alltrim(strzero((tmp1)->numvol,3)),"N","0","050,028") // CAIXA
			...
			mscbsay(009,010,alltrim(_ntitulo),"N","0","050,028") // TÍTULO
			mscbsay(010,034,"Fab.: "+(tmp1)->fabric,"N","0","050,022")               // FABRICANTE        // Guilherme 16/09/2015   
			mscbsay(055,034,"Lote Fab.: "+(tmp1)->lotfabr,"N","0","050,022")        //  LOTE FABRICANTE   // Guilherme 16/09/2015
			...
			/* Inicio Leandro - 07/12/16 */ 
			
			_Qry := "SELECT MAX (B8_DTVALID) DTVALID "
			_Qry += "FROM "+RetSqlName("SB8")+" SB8 "
			_Qry += "WHERE SB8.D_E_L_E_T_ <> '*' AND b8_lotectl = '"+(tmp1)->lotectl+"'" 
			...
			DbUseArea(.T.,"TOPCONN",TcGenQry(,,ChangeQuery(_Qry)),"TEMP",.T.,.T.)
			
			_dataval := STOD(TEMP->DTVALID)
			...
			if alltrim(_crevalida)=="Nao"
				...
				mscbsay(055,029,"Dt.Reanalise: "+dtoc(stod((tmp1)->dtvalid)-180),"N","0","050,022") // REANÁLISE
			else
				//mscbsay(055,023,"Dt.Val.: "+dtoc(sb8->b8_dtvalid),"N","0","050,035") // DATA DE VALIDADE -- Leandro - 07/12/16
				mscbsay(055,023,"Dt.Val.: "+dtoc(_dataval),"N","0","050,035") // DATA DE VALIDADE
				mscbsay(009,029,"Dt.Fab.: "+dtoc(stod((tmp1)->dtfabr)),"N","0","050,022") // DATA DE FABRICACAO
				mscbsay(055,029,"Dt.Reanalise: "+dtoc(sb8->b8_dtvalid-180),"N","0","050,022") // REANÁLISE
			endif			
		...
		mscbsay(009,010,alltrim(_ntitulo),"N","0","050,028") // TÍTULO
		mscbsay(010,034,"Fab.: "+(tmp1)->fabric,"N","0","050,022")               // FABRICANTE        // Guilherme 16/09/2015   
		mscbsay(060,034,"Lote Fab.: "+(tmp1)->lotfabr,"N","0","050,022")        //  LOTE FABRICANTE   // Guilherme 16/09/2015 
		mscbsay(011,017,left((tmp1)->cod,6)+"-"+(tmp1)->descri,"N","0","050,035") // CODIGO + DESCRICAO DO PRODUTO
		...
		mscbsay(009,028,"Dt.Fab.: "+dtoc(stod((tmp1)->dtfabr)),"N","0","050,022") // DATA DE FABRICACAO 
		mscbsaybar(005,040,_ccodbar,"N","C",15,.F.,.F.,.F.,,2,1,.T.,.F.,"1",.T.)
		mscbsay(020,055,_ccodimp,"N","0","030,015")
		mscbend()
	endif
