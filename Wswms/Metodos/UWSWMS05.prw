...
ฑฑบDesc.     ณMetodo WS para consulta de Saldo Estoque                    บฑฑ
ฑฑบ          ณ                                                            บฑฑ
...
User Function UWSWMS05(oFil, oRet)
	...
	if lOK
		...
		if !empty(cArmazens)
			if cMv_CQ $ cArmazens //se armaz้m do CQ estiver dentro do filtro
		...
		cQry += CRLF + "          AND SB8.B8_QACLASS  > 0 " //somente com saldo a endere็ar
		...
		//retirei porque a consulta precisa trazer todos os produtos เ endere็ar, mesmo em armaz้m diferente do CQ
		...
		if QRYWS5->(Eof())
			cMsgErr := "Produto nใo localizado no Endere็o."
			lOK := .f.
			nQtdReg := 0
		else
			While QRYWS5->(!Eof())
				if len(aEnderecos) == 0 .or. AScan(aEnderecos, {|x| AllTrim(x) == AllTrim(QRYWS5->ENDERECO)}) > 0
					...
					if SBE->BE_MSBLQL == "1" .and. At("Endere็o (" + AllTrim(QRYWS5->ENDERECO) + ")", cMsgErr) == 0
						cMsgErr	+= iif(empty(cMsgErr), "", CRLF) + "Endere็o (" + AllTrim(QRYWS5->ENDERECO) + ") bloqueado."
						QRYWS5->(DbSkip())
						loop
					...
					*/
					...
					// Verifica็ใo se o lote tem origem na SC2 ou SD1 para pegar data fabri็ใo
