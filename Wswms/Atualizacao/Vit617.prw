...
/*/{Protheus.doc} Vit617
	Função para importar registros do inventáio. Execauto MATA270 
@author Microsiga
@since 19/06/17
@version 1.0
...
@type function
/*/
User Function Vit617()

	Private cPerg := "Vit617    "

	If !ValidPerg() .or. Empty(mv_Par01) .or. Empty(mv_Par02) .or. Empty(mv_Par03)
		MsgStop("Informe os parâmetros para a importação de planilha para invetário...", "Atenção")
		Return()
	EndIf
	
	If !( ".CSV" $ Upper(mv_Par01) )
		MsgStop("Selecione um arquivo válido (extensão .csv) para a importação da planilha...", "Atenção")
		Return()
	EndIf
	
	Processa({|lEnd| ImpPlanilha()},"Importando de planilha para invetário...")
...
if nPosPrd == 0
	msgStop("Não achou a coluna dos produtos...")
	return
elseif nPosArm == 0
		msgStop("Não achou a coluna dos armazens...")
		return
elseif nPosEnd == 0
		msgStop("Não achou a coluna dos endereços...")
		return
elseif nPosLote == 0
		msgStop("Não achou a coluna dos lotes...")
		return
elseif nPosQtde == 0
		msgStop("Não achou a coluna de quantidade...")
		return
endif
...
cLog := "Erros gerados na importação"    
...
For nLin := 1 to Len(aDados)
	...
	if !dbSeek(XFilial("SB1")+cProd)
		cLog += cLinErr + "Produto (" + cProd + ") não localizado no cadastro..."
		loop
	elseif SB1->B1_LOCALIZ <> "S"
			cLog += cLinErr + "Produto (" + cProd + ") não controla endereço..."
			loop
	endif
	...
	if !dbSeek(XFilial("SBE")+cArmaz+cEnder)
		cLog += cLinErr + "Endereço (" + cEnder + "), armazém ("+cArmaz+") não localizado no cadastro..."
		loop
	elseif SBE->BE_MSBLQL <> "2"
			cLog += cLinErr + "Endereço (" + cEnder + "), armazém ("+cArmaz+") bloqueado no cadastro..."
			loop
	endif
...
PutSx1(cPerg,"02",OemToAnsi("Data do Inventário ?"),"","","mv_ch2","D",08,0,0,"G","","","","","mv_par02","","","","","","","","","","","","","","","","",aHelpPor,{},{})
PutSx1(cPerg,"03",OemToAnsi("Doc. do Inventário ?"),"","","mv_ch3","C",TamSX3("B7_DOC")[1],0,0,"G","","","","S","mv_par03","","","","","","","","","","","","","","","","",aHelpPor,{},{})
PutSx1(cPerg,"04",OemToAnsi("Zerar Inventário   ?"),"","","mv_ch4","N",01,0,0,"C","","","","S","mv_par04","Sim","","","2","Não","","","","","","","","","","","",aHelpPor,{},{})
