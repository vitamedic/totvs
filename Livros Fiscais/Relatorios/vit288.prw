...
±±³Programa  ³ VIT288   ³ Autor ³ Heraildo C. de Freitas³ Data ³ 28/12/06 ³±±
...
nomeprog :="VIT288"
wnrel    :="VIT288"+Alltrim(cusername)
aordem   :={}
alinha   :={}
nlastkey :=0
lcontinua:=.t.
...
if prow()>0
	...
	while _dfisini<=_ddatafim
		...
		// BUSCA VALOR CONTÁBIL DE EXPORTAÇÕES E INCLUI COMO BASE ICMS
		_cquery:=" SELECT"
		_cquery+=" SUM(F3_VALCONT) F3_VALCONT"
...
_cquery+=" SELECT "
_cquery+=" FA_CODIGO CODIGO,"
_cquery+=" FA_DATA DTMOV,"
_cquery+=" F9_DOCNFS NOTA,"
_cquery+=" F9_DESCRI DESCRI,"
_cquery+=" 0 VALOR,"
_cquery+=" FA_VALOR BAIXA "
_cquery+=" FROM "
_cquery+= retsqlname("SFA")+" SFA,"
_cquery+= retsqlname("SF9")+" SF9"
_cquery+=" WHERE "
_cquery+=" SFA.D_E_L_E_T_=' '"
_cquery+=" AND SF9.D_E_L_E_T_=' '"
_cquery+=" AND FA_TIPO='2'"
_cquery+=" AND FA_DATA BETWEEN '"+dtos(_ddataini)+"' AND '"+dtos(_ddatafim)+"'"
_cquery+=" AND FA_CODIGO=F9_CODIGO"
_cquery+=" AND F9_DTENTNE BETWEEN '"+dtos(_ddataini)+"' AND '"+dtos(_ddatafim)+"'"
_cquery+=" ORDER BY"
_cquery+=" 2,1,6,3"
