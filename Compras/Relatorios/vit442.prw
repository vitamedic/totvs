...
±±³Programa  ³ VIT442   ³ Autor ³ Guilherme Teodoro     ³ Data ³ 02/09/15 ³±±
...
user function vit442()
nordem  :=""
tamanho :="G"
limite  :=220
titulo  :="ENTRADAS DE INSUMOS E EMBALAGENS POR FONECEDOR E FABRICANTE"
cdesc1  :="Este programa ira emitir uma lista de NFs lancadas por Periodo, Fornecedores e Fabricantes"
cdesc2  :=""
cdesc3  :=""
cstring :="SF1"
areturn :={"Zebrado",1,"Administracao",1,2,1,"",1}
nomeprog:="VIT442"
wnrel   :="VIT442"+Alltrim(cusername)
alinha  :={}
aordem  :={}
nlastkey:=0
...
cabec2:="CODIGO  PRODUTO                                       QUANTIDADE  UM  FORNECEDOR                               C.G.C.         LT.FORNECEDOR      N.F.    ENTRADA  LT.VITAMEDIC  FABRICANTE                     LT.FABRIC."
...
while ! tmp1->(eof()) .and.;
	...
	@ prow(),065   PSAY tmp1->um
	@ prow(),068   PSAY tmp1->fornece
	@ prow(),110   PSAY tmp1->cgc
	@ prow(),126   PSAY tmp1->lotefor
	@ prow(),142   PSAY tmp1->doc
	@ prow(),152   PSAY dtoc(tmp1->dtdigit)
	@ prow(),164   PSAY tmp1->lote
	@ prow(),175   PSAY tmp1->fabric
	@ prow(),208   PSAY tmp1->lotfabr
...
_cquery+=" AND F4_ESTOQUE='S'"
_cquery+=" AND D1_LOCAL NOT IN ('01','15','30','40')" //Guilherme - 30/06/2017 - Tratativa para não contemplar as devoluções de terceiros.
