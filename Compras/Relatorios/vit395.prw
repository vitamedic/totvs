...
±±³Programa  ³ VIT395 ³ Autor ³ Guilherme Teodoro Teles ³ Data ³ 08/04/14 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descricao ³ GERA PLANILHA ACOMPANHAMENTO DE COMPRAS EXPORTANDO PARA    ³±±
±±³          ³ EXCEL, CONFORME PARÂMETROS INFORMADOS, MELHORIA VIT342     ³±±
...
User Function VIT395()     


Private _cperg := "PERGVIT395"
...
While !tmp1->(Eof())         
	...
	AAdd(_aDados, {tmp1->numsc, tmp1->itsc, tmp1->prod, tmp1->descri,tmp1->arm,tmp1->qt_sc,tmp1->qt_at_sc,(tmp1->qt_sc-tmp1->qt_at_sc),;
   				...
   				tmp1->reem_sc,tmp1->num_pc,tmp1->it_pc,tmp1->qt_pc,tmp1->qt_at_pc,(tmp1->qt_pc-tmp1->qt_at_pc),tmp1->um_pc,tmp1->vlr_pc,tmp1->vlrtot_pc,tmp1->moeda,tmp1->txmoeda,tmp1->ultprc,;
...
tcsetfield("TMP1","VLR_PC"    ,"N",12,8)
tcsetfield("TMP1","VLRTOT_PC" ,"N",12,2)
tcsetfield("TMP1","MOEDA"     ,"N",12,2) 
tcsetfield("TMP1","TXMOEDA"   ,"N",12,2)
