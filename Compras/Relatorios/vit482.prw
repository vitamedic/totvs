...
/*/
User Function vit482( cAlias, nReg, nOpcx )
...
While !oReport:Cancel() .And. !SC7->(Eof()) .And. SC7->C7_FILIAL == xFilial("SC7") .And. SC7->C7_NUM >= mv_par01 .And. SC7->C7_NUM <= mv_par02
	...
	For nVias := 1 to mv_par09
		...
		While !oReport:Cancel() .And. !SC7->(Eof()) .And. SC7->C7_FILIAL == xFilial("SC7") .And. SC7->C7_NUM == cNumSC7
			...
			If AllTrim(mv_par06) == "B1_DESC"
				...
				cDescPro := alltrim(SB1->B1_DESC) + " - Cons.Med.: " + cValtoChar(tmp1->CONSUMO)
			ElseIf AllTrim(mv_par06) == "B5_CEME"
				SB5->(dbSetOrder(1))
				If SB5->(dbSeek( xFilial("SB5") + SC7->C7_PRODUTO ))
					cDescPro := alltrim(SB5->B5_CEME)+" - Cons.Med.: " + cValtoChar(tmp1->CONSUMO)
				EndIf
			ElseIf AllTrim(mv_par06) == "C7_DESCRI"
				cDescPro := alltrim(SC7->C7_DESCRI) +" - Cons.Med.: " + cValtoChar(tmp1->CONSUMO)
			EndIf
			...
			If SA5->(dbSeek(xFilial("SA5")+SC7->C7_FORNECE+SC7->C7_LOJA+SC7->C7_PRODUTO)) .And. !Empty(SA5->A5_CODPRF)
				cDescPro := Alltrim(cDescPro) + " ("+Alltrim(SA5->A5_CODPRF)+")" //+ " - Cons.Med.: " + cValtoChar(tmp1->CONSUMO)
			EndIf
	...
	//Incluida validação para os pedidos de compras por item do pedido  (IP/alçada)			
		...
		If !lNewAlc
			...
			oReport:Box( 2700 , 1220 , 3020 , 1620 )
			oReport:Box( 2700 , 1620 , 3020 , 2020 )
			oReport:Box( 2700 , 2020 , 3020 , nPageWidth )
...
//			oReport:Box( 2600 , 1770 , 3020 , nPageWidth )
			...
			oReport:PrintText( "Superintendencia" ,nLinPC, 1280) 
			oReport:PrintText( "Financeiro" ,nLinPC, 1650)
			oReport:PrintText( If((SC7->C7_TIPO == 1 .OR. SC7->C7_TIPO == 3),STR0079,STR0084),nLinPC,2050) //"Liberacao do Pedido"##"Liber. Autorizacao "
//			oReport:PrintText( STR0080 + IF( SC7->C7_TPFRETE $ "F","FOB",IF(SC7->C7_TPFRETE $ "C","CIF",IF(SC7->C7_TPFRETE $ "T","Por Conta Terceiros"," " ) )) ,nLinPC,1820 )
			...
			oReport:PrintText( Replic("_",23) ,nLinPC, 1280 )
			oReport:PrintText( Replic("_",23) ,nLinPC, 1640 )
			oReport:PrintText( Replic("_",23) ,nLinPC, 2050 )
			oReport:SkipLine()
			oReport:SkipLine()
			If SC7->C7_TIPO == 1 .OR. SC7->C7_TIPO == 3
			...
			EndIf
//			oReport:SkipLine()
				oReport:PrintText("VITAMEDIC NÃO REALIZA PAGAMENTOS DE TITULOS DESCONTADOS EM FACTORING, FOMENTO OU TERCEIROS.",,050 )
			oReport:SkipLine()
			oReport:SkipLine()
			oReport:SkipLine()
			oReport:SkipLine()
...
//		@ 09,065 PSAY IIF(nDifColcc == 0,STR0056,STR0057)	//"|Valor Unitario|      Valor Total   |Entrega   |  C.C.   | S.C. |"
		@ 09,065 PSAY IIF(nDifColcc == 0,STR0056,STR0057)+"Cons|"	//"|Valor Unitario|      Valor Total   |Entrega   |  C.C.   | S.C. |"
	Else
//		@ 09,065 PSAY IIF(nDifColcc == 0,STR0047,STR0055)	//"|Valor Unitario|IPI% |  Valor Total   | Entrega  |  C.C.   | S.C. |"
		@ 09,065 PSAY IIF(nDifColcc == 0,STR0047,STR0055)+"Cons|"	//"|Valor Unitario|IPI% |  Valor Total   | Entrega  |  C.C.   | S.C. |"
	EndIf
	@ 10,001 PSAY "|"
