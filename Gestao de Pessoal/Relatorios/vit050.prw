...
ÛÛ³Programa ³  VIT050   ³Autor ³Gardenia		        ³ Data ³ 08/03/2002 ³ÛÛ
ÛÛÃÄÄÄÄÄÄÁÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´ÛÛ
...
aLinha   := {}
m_pag    := 0
wnRel    := "VIT050"+Alltrim(cusername)
...
Do While !Eof()
  ...
  IncRegua()
	...
	_passou :=.t.
	Do While !Eof() .and. sra->ra_cc == _cc
	  If sra->ra_sitfolh =='D'
	    dbSelectArea("SRA")
	    dbSkip()
	    Loop
	  ...
	  if  _dias <30 .or. (!empty(srf->rf_dataini))
	   ...
	   if mv_par05 =1
		   if _passou
				cabec(titulo,cabec1,cabec2,nomeprog,tamanho,ntipo)
				@ prow()+2,00 PSAY _cc + " - " + si3->i3_desc
				_passou :=.f.
	      endif
	   elseif mv_par05=2 .and. _passou   
				@ prow()+2,00 PSAY _cc + " - " + si3->i3_desc
				_passou :=.f.
	   endif   
		if  prow()>55
