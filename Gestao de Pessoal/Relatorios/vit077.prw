...
tamanho  :="P"
limite   :=80
titulo   :="FICHA DE DECLARAÇÃO DE OPÇÃO"
...
while ! tmp1->(eof()) .and.;
			lcontinua
	@ prow()+4,000 PSAY "                           DECLARAÇÃO DE OPÇÃO"
	@ prow()+6,010 PSAY "     Eu "+alltrim(tmp1->nome)+","
	@ prow()+2,005 PSAY "portador(a) da Carteira profissional nº"+tmp1->numcp+"/"+tmp1->sercp+tmp1->ufcp+", empregado(a) da "
	if sm0->m0_codigo=='01'
		@ prow()+2,005 PSAY "empresa  VITAMEDIC INDÚSTRIA FARMACÊUTICA LTDA. sita à Rua VPR 01 Qd. 2-A"
		@ prow()+2,005 PSAY "Modulo  01 - DAIA, Anápolis-GO, declaro, para todos os fins, que, nesta"
	elseif sm0->m0_codigo=='03'
		@ prow()+2,005 PSAY "empresa FUNDAÇÃO CULTURAL VITAMEDIC sita à Av. Pedro  Ludovico  Lt. 19/20"
		@ prow()+2,005 PSAY "Jd. Ana Paula ,  Anápolis-GO,  declaro,  para todos os fins, que, nesta"
	endif	
	@ prow()+2,005 PSAY "data, exerço a opção pelo regime do Regulamento do Fundo de Garantia do"
	@ prow()+2,005 PSAY "Tempo de Serviço"
   ...
   @ prow()+3,005 PSAY "1ª) _______________________________________________"
   @ prow()+3,005 PSAY "2ª) _______________________________________________"
	@ prow()+3,005 PSAY "___________________________________________________"
	@ prow()+1,005 PSAY "     (Assistente responsável legal pelo menor)"
	tmp1->(dbskip())
	if  ! tmp1->(eof())
		@ 66,00 PSAY " "
		@ 00,00 PSAY " "
