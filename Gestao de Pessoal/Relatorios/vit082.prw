...
while ! tmp1->(eof()) .and.;
			lcontinua
	@ prow()+4,000 PSAY "                     DECLARAÇÃO DE RENÚNCIA DO VALE TRANSPORTE"
	@ prow()+8,005 PSAY alltrim(sm0->m0_cidcob)+", "+str(day(ddatabase),2)+" de "+;
						mesextenso(month(ddatabase))+" de "+str(year(ddatabase),4)
	@ prow()+5,005 PSAY "A"       
	if sm0->m0_codigo=="01"	
		@ prow()+1,005 PSAY "VITAMEDIC INDÚSTRIA FARMACÊUTICA LTDA."
		@ prow()+1,005 PSAY "Rua: VPR 01 Qd. 2-A Módulo 01"
	...
	@ prow()+8,005 PSAY "     Declaro para  os devidos  fins,  que  não  pretendo  fazer  uso  do"
	@ prow()+1,005 PSAY "programa Vale Tranporte, instituído em  lei, e portanto, renuncio a este"
	@ prow()+1,005 PSAY "benefício."
