...
±±³Programa  ³ VIT132   ³ Autor ³ Gardenia              ³ Data ³ 15/07/02 ³±±
...
nomeprog :="VIT132"
wnrel    :="VIT132"+Alltrim(cusername)
alinha   :={}
nlastkey :=0
cbcont   :=0
m_pag    :=1
...
//cabec1:="Função                             Nome                                       Salário              Comissão  Sub-Total Encargos(1)   FGTS(2)  Sub-Total   Vale Tr.   Refeição     Unimed     Total"
 cabec1:="Função                                  Nome                                       Salario        Comissao   Sub-Total Encargos(1)   FGTS(2)  Sub-Total   Vale Tr.   Refeição     Unimed     Total"
...
//XXXXXXXXXXXXXXXXXXXX XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX  99,999.99 99,999.99 99,999.99 99,999.99 99,999.99 99,999.99 99,999.99 99,999.99 99,999.99 99,999.99 99,999.99	
		_sal1:=0
		src->(dbseek(_cfilsrc+_cc+tmp1->mat+"710"+"   "))
		_sal1:=src->rc_valor
		src->(dbseek(_cfilsrc+_cc+tmp1->mat+"115"+"   "))
		_sal1+=src->rc_valor
		src->(dbseek(_cfilsrc+_cc+tmp1->mat+"298"+"   "))
		_sal1+=src->rc_valor
		src->(dbseek(_cfilsrc+_cc+tmp1->mat+"107"+"   "))
		_sal1+=src->rc_valor
		src->(dbseek(_cfilsrc+_cc+tmp1->mat+"113"+"   "))
		_sal1+=src->rc_valor
		src->(dbseek(_cfilsrc+_cc+tmp1->mat+"104"+"   "))
		_sal1-=src->rc_valor
      _comis:=src->rc_valor
		...
		if _sal1 >0
   		...
   		@ prow(),004 PSAY substr(si3->i3_desc,1,30)
			@ prow(),040 PSAY substr(tmp1->nome,1,35)
			@ prow(),080 PSAY _sal1 picture "@E 999,999.99"
//			@ prow(),075 PSAY _assi1 picture "@E 999,999.99"
//	@ prow(),086 PSAY tmp1->extra picture "@E 999,999.99"
			@ prow(),095 PSAY _comis picture "@E 999,999.99"
			_subtot1:=_sal1+tmp1->extra+_comis
			@ prow(),108 PSAY _subtot1 picture "@E 999,999.99"
			src->(dbseek(_cfilsrc+_cc+tmp1->mat+"706"+"   "))
			_encargos:=src->rc_valor*(mv_par05/100)
			@ prow(),119 PSAY _encargos picture "@E 999,999.99"
			src->(dbseek(_cfilsrc+_cc+tmp1->mat+"711"+"   "))
			_fgts1:=src->rc_valor
			src->(dbseek(_cfilsrc+_cc+tmp1->mat+"810"+"   "))
			_fgts2:=src->rc_valor
			@ prow(),130 PSAY _fgts1+_fgts2  picture "@E 999,999.99"
			_subtot2:=_subtot1+_encargos +(_fgts1+_fgts2) 
			@ prow(),141 PSAY _subtot2 picture "@E 999,999.99"
			src->(dbseek(_cfilsrc+_cc+tmp1->mat+"553"+"   "))
			_trans:=(src->rc_horas*2*mv_par07)-src->rc_valor
			@ prow(),153 PSAY _trans picture "@E 999,999.99"
			src->(dbseek(_cfilsrc+_cc+tmp1->mat+"552"+"   "))
			_ref:=src->rc_valor/mv_par09
			_ref:=(_ref*mv_par06)-src->rc_valor
			@ prow(),164 PSAY _ref picture "@E 999,999.99"
			srx->(dbseek("22"+"01"+strzero(val(tmp1->asmedic),2,0)))
			src->(dbseek(_cfilsrc+_cc+tmp1->mat+"551"+"   "))
//		_taxaunimed:=val(substr(srx->rx_txt,45,2)/100
			_valunimed:=val(substr(srx->rx_txt,27,6))
			_unimed:=_valunimed*(val(tmp1->dpassme)+1)                         
//		msgstop(_valunimed)
			_unimed:=_unimed - src->rc_valor
	...
	if _cctotsal >0
		...
		@ prow(),080 PSAY _cctotsal picture "@E 999,999.99"
//@ prow(),086 PSAY _cctotvalb picture "@E 999,999.99"
      @ prow(),095 PSAY _cctotcomis picture "@E 999,999.99"
...
// prow(),086 PSAY _totvalb/_tottotal*100 picture "@E 999,999.99"
   @ prow(),095 PSAY _totcomis/_tottotal*100 picture "@E 999,999.99"
...
@ prow(),040 PSAY substr(sra->ra_nome,1,35)
...
@ prow(),080 PSAY mv_par08 picture "@E 999,999.99"
@ prow(),186 PSAY mv_par08 picture "@E 999,999.99" 
...
@ prow(),080 PSAY _totsal+_totpro+mv_par08 picture "@E 999,999.99"
//prow(),075 PSAY _totassid picture "@E 999,999.99"
//prow(),086 PSAY _totvalb picture "@E 999,999.99"
@ prow(),095 PSAY _totcomis picture "@E 999,999.99"
...
@ prow(),164 PSAY _totref picture "@E 999,999.99"
@ prow(),175 PSAY _totunimed picture "@E 999,999.99"
@ prow(),186 PSAY _tottotal+_totpro+_totproenc+mv_par08 picture "@E 999,999.99"

@ prow()+1,004 PSAY "Totais (%)=>"
@ prow(),080 PSAY (_totsal+_totpro+mv_par08)/(_tottotal+_totpro+mv_par08)*100 picture "@E 999,999.99"
...
@ prow(),095 PSAY _totcomis/(_tottotal+_totpro)*100 picture "@E 999,999.99"
...
@ prow(),186 PSAY (_tottotal+_totpro+_totproenc+mv_par08)/(_tottotal+_totpro+_totproenc+mv_par08)*100 picture "@E 999,999.99"
...
aadd(_agrpsx1,{cperg,"08","Serv. Terceiros    ?","mv_ch8","N",12,2,0,"G",space(60),"mv_par08"       ,space(15)        ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),space(15),space(15)        ,space(30),"   "})
