...
±±³Programa  ³ VIT085   ³ Autor ³ Gardenia Ilany        ³ Data ³ 21/06/02 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descricao ³ Aviso Previo do Empregador para Dispensa do Colaborador    ³±±
...
nomeprog :="VIT085"
wnrel    :="VIT085"+Alltrim(cusername)
alinha   :={}
nlastkey :=0
lcontinua:=.t.
...
while ! tmp1->(eof()) .and.;
   ...
   if _dias <=30
      msgstop("Funcionário "+Alltrim(tmp1->nome) + " com estabilidade de férias !!!")
//    tmp1->(dbskip())      
//		loop
	endif	
	@ prow()+4,000 PSAY "            AVISO PRÉVIO DO EMPREGADOR PARA DISPENSA DO COLABORADOR "
	@ prow()+4,005 PSAY "Ao"
	@ prow()+1,005 PSAY "Sr(a): "+ tmp1->nome
	@ prow()+1,005 PSAY "NESTA"                     
	@ prow()+4,005 PSAY "Pelo presente o notificamos que não mais serão utilizados os seus serviços"
	@ prow()+1,005 PSAY "pela nossa empresa e por isso vimos avisá-lo nesta data, nos termos e para"
	@ prow()+1,005 PSAY "os efeitos do disposto no Artigo 487, Item II, Cap. VI, Título IV do"
	@ prow()+1,005 PSAY "Decreto Lei nº 5452 de 01/05/1943 (CLT). "
	@ prow()+1,005 PSAY "O aviso prévio de "+transform(mv_par04,'99')+ " dias será,"
	@ prow()+3,005 PSAY "Opção quando aviso for trabalhado:"
	@ prow()+2,005 PSAY "[  ] 1ª. opção: Declaro-me ciente, optando pela redução de 02 (duas) horas" 
	@ prow()+1,005 PSAY "diáras."
	@ prow()+2,005 PSAY "[  ] 2ª. opção: Declaro-me ciente, optando  pela  ausência  no trabalho de" 
	@ prow()+1,005 PSAY " ____________(___________) dias corridos."
	@ prow()+1,005 PSAY "Aviso Prévio nas verbas rescisórias."
	@ prow()+3,005 PSAY "Pedimos a devolução da presente com seu 'CIENTE'."
	...
	@ prow()+1,005 PSAY "rescisão. Para tanto favor comparecer  na data acima,  munido de seus" 
	@ prow()+1,005 PSAY "documentos."
	tmp1->(dbskip())
	if  ! tmp1->(eof())
		@ 66,00 PSAY " "
		@ 00,00 PSAY " "
